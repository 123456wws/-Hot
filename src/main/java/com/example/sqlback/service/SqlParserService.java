...
public class SqlParserService {
    ...
    private TableInfo parseCreateTable(CreateTable createTable) {
        ...
        if (createTable.getIndexes() != null) {
            ...
            log.info("表{}的索引对象数量: {}", tableName, createTable.getIndexes().size());
            
            for (Object index : createTable.getIndexes()) {
                log.info("处理索引对象: 类型={}", index.getClass().getName());
                ...
                if (index instanceof ForeignKeyIndex) {
                    ...
                    log.warn("索引对象不是外键: {}", index);
                }
            }
        } else {
            log.info("表{}没有索引对象", tableName);
        }
